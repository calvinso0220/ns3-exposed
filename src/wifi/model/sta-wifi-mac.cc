...
StaWifiMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::StaWifiMac")
    ...
    .AddAttribute ("ActiveProbing",
                   ...
                   MakeBooleanChecker ())
	.AddAttribute ("HelloInterval", "Delay between two Hellos",
					TimeValue(MicroSeconds(1024000)),
					MakeTimeAccessor(&StaWifiMac::GetHelloInterval, &StaWifiMac::SetHelloInterval), MakeTimeChecker())
	.AddAttribute ("HelloJitter",
					"A uniform random variable to randomize hello sending time.",
					StringValue("ns3::UniformRandomVariable"),
					MakePointerAccessor(&StaWifiMac::m_helloJitter),
					MakePointerChecker<UniformRandomVariable>())
	.AddAttribute ("EnableHelloJitter",
					"If Hellos are enabled, whether to jitter the initial send event.",
					BooleanValue(true),
					MakeBooleanAccessor(&StaWifiMac::m_enableHelloJitter),
					MakeBooleanChecker())
	.AddAttribute ("HelloGeneration", "Send Periodic Hello",
					BooleanValue(false),
					MakeBooleanAccessor(&StaWifiMac::SetHelloGeneration, &StaWifiMac::GetHelloGeneration), MakeBooleanChecker())
...
void StaWifiMac::DoInitialize(void) {
	...
	m_neighborManager = CreateObject<NeighborInfoManager>();
	m_neighborManager->Initialize(GetAddress());

	m_helloEvent.Cancel();
	if(m_enableHelloGeneration) {
		if(m_enableHelloJitter) {
			int64_t jitter = m_helloJitter->GetValue(m_helloInterval.GetMicroSeconds(), 2*m_helloInterval.GetMicroSeconds());
			m_helloEvent = Simulator::Schedule(MicroSeconds(jitter), &StaWifiMac::SendOneHello, this);
		} else {
			m_helloEvent = Simulator::Schedule(m_helloInterval, &StaWifiMac::SendOneHello, this);
		}
	}
...
//NETSYS -----------------------------------------------------------------------
void
StaWifiMac::PrintNeighborStatus() {
	m_neighborManager->PrintTable();
}

void
StaWifiMac::SetHelloGeneration(bool enable) {
	NS_LOG_FUNCTION(this << enable);
	if(!enable) {
		m_helloEvent.Cancel();
	}
	else if (enable && !m_enableHelloGeneration) {
		m_helloEvent = Simulator::ScheduleNow(&StaWifiMac::SendOneHello, this);
	}
	m_enableHelloGeneration = enable;
}

bool
StaWifiMac::GetHelloGeneration(void) const {
	NS_LOG_FUNCTION(this);
	return m_enableHelloGeneration;
}

void 
StaWifiMac::SetHelloInterval(Time interval) {
	NS_LOG_FUNCTION(this << interval);
	m_helloInterval = interval;
}

Time
StaWifiMac::GetHelloInterval(void) const {
	NS_LOG_FUNCTION(this);
...
StaWifiMac::AssignStreams(int64_t stream) {
	...
	m_helloJitter->SetStream(stream);
	return 1;
}
//------------------------------------------------------------------------------
...
// NETSYS ----------------------------------------------------------------------
void 
StaWifiMac::SendOneHello(void) {
	//NS_LOG_UNCOND(Simulator::Now() << " " << GetAddress() << " SendHello");
	...
	hdr.SetDsNotTo();
	Ptr<Packet> packet = Create<Packet>();
	MgtHelloHeader hello;
	hello.SetNeighborAPInformation(m_neighborManager->GetNeighborAPInformation());
...
StaWifiMac::TryToEnsureAssociated (void)
{
  ...
  switch (m_state)
    ...
    case BEACON_MISSED:
      ...
      m_linkDown ();
//NETSYS
		m_neighborManager->Deassociated();
  ...
  if (hdr->GetAddr3 () == GetAddress ())
           ...
           || hdr->IsAssocReq ())
    {
      ...
      return;
		...
		Mac48Address sta_addr = hdr->GetAddr2();
		Mac48Address ap_addr = hdr->GetAddr3();
		NeighborAPInformation neighbor_aps = hello.GetNeighborAPInformation();

		NS_LOG_UNCOND("----------------");
		NS_LOG_UNCOND(Simulator::Now() << " " << GetAddress() << " HELLO RECEIVED");
		NS_LOG_UNCOND("sta_addr: " << sta_addr);
		NS_LOG_UNCOND("ap_addr : " << ap_addr);
		NS_LOG_UNCOND("neighboring APs");
		for(unsigned i=0; i<neighbor_aps.GetNAPs(); i++) {
			NS_LOG_UNCOND(neighbor_aps.m_APs[i]);	
		}
		NS_LOG_UNCOND("----------------");
	}
    ...
    {
		//NS_LOG_UNCOND(Simulator::Now() << " " << GetAddress() << " Received Beacon from AP: " << hdr->GetAddr2());
      ...
      if (GetSsid ().IsBroadcast ()
          || beacon.GetSsid ().IsEqual (GetSsid ()))
        {
          ...
          //NS_LOG_UNCOND ("Beacon is for our SSID");
          goodBeacon = true;
        }
...
//NETSYS
		m_neighborManager->BeaconReceived(hdr->GetAddr3());		
          ...
          if (assocResp.GetStatusCode ().IsSuccess ())
            {
              SetState (ASSOCIATED);
//NETSYS
				m_neighborManager->Associated(hdr->GetAddr3());
