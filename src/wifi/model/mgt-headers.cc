...
// NETSYS
NS_OBJECT_ENSURE_REGISTERED(MgtHelloHeader);

MgtHelloHeader::MgtHelloHeader() { } 
MgtHelloHeader::~MgtHelloHeader() { }

NeighborAPInformation MgtHelloHeader::GetNeighborAPInformation(void) const {
	return m_aps;
}

void MgtHelloHeader::SetNeighborAPInformation(NeighborAPInformation aps) {
	m_aps = aps;
}

TypeId MgtHelloHeader::GetTypeId(void) {
	static TypeId tid = TypeId("ns3::MgtHelloHeader")
		.SetParent<Header> ()
		.SetGroupName("Wifi")
		.AddConstructor<MgtHelloHeader> ()
	;
	return tid;
}

TypeId MgtHelloHeader::GetInstanceTypeId(void) const {
	return GetTypeId();
}

uint32_t MgtHelloHeader::GetSerializedSize(void) const {
	uint32_t size = 0;
	size += m_aps.GetSerializedSize();
	return size;
}

void MgtHelloHeader::Print(std::ostream &os) const {
	// not implemented yet
	os << "hello header";
}

void MgtHelloHeader::Serialize(Buffer::Iterator start) const {
	Buffer::Iterator i = start;
	i = m_aps.Serialize(i);
}

uint32_t MgtHelloHeader::Deserialize(Buffer::Iterator start) {
	Buffer::Iterator i = start;
	i = m_aps.Deserialize(i);
	return i.GetDistanceFrom(start);
}
