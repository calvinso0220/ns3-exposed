...
YansWifiPhy::StartReceivePreambleAndHeader (Ptr<Packet> packet,
                                            ...
                                            Time rxDuration)
{
	//NS_LOG_UNCOND(Simulator::Now() << " YansWifiPhy::StartReceivePreambleAndHeader " << m_device->GetNode()->GetId());
  ...
  switch (m_state->GetState ())
    ...
    case YansWifiPhy::IDLE:
      ...
      if (rxPowerW > GetEdThresholdW () && continueRx) //checked here, no need to check in the payload reception (current implementation assumes constant rx power over the packet duration)
        {
...
YansWifiPhy::StartReceivePacket (Ptr<Packet> packet,
                                 ...
                                 Ptr<InterferenceHelper::Event> event)
{
	//NS_LOG_UNCOND(Simulator::Now() << " YansWifiPhy::StartReceivePacket " << m_device->GetNode()->GetId());
	...
	WifiMacHeader hdr;
	packet->PeekHeader(hdr);
	//NS_LOG_UNCOND("I am " << m_self << " This packet is destined to " << hdr.GetAddr1());
	if(hdr.GetAddr1().IsGroup() == false && hdr.GetAddr1() != m_self) {
        //NS_LOG_UNCOND(Simulator::Now() << " Cancel Reception " << m_device->GetNode()->GetId());
		NotifyRxDrop(packet);
		m_state->SwitchFromRxCanceled(packet);
		m_endRxEvent.Cancel();
		return;
	}
	//--------------------------------------------------------------------------
...
{
	//NS_LOG_UNCOND(Simulator::Now() << " YansWifiPhy::EndReceive " << m_device->GetNode()->GetId());
  ...
  if (m_plcpSuccess == true)
    {
      ...
      //NS_LOG_UNCOND ("mode=" << (event->GetPayloadMode ().GetDataRate (event->GetTxVector ())) <<
       //            ", snr(dB)=" << RatioToDb (snrPer.snr) << ", per=" << snrPer.per << ", size=" << packet->GetSize ());
